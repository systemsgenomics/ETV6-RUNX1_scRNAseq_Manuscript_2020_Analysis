# Run pySCENIC for HCA B lineage and leukemic cells from scanpy analysis.
...
## Set paths
data_folder = '/research/groups/allseq/data/scRNAseq/results/juha_wrk/SCENIC/HCA_ALL_jitter/'
...
final_regulons_aucell_means_fname = os.path.join(data_folder, 'aucell_final_regulons_means.csv.gz')
final_regulons_aucell_means_rss_fname = os.path.join(data_folder, 'aucell_final_regulons_means_rss.csv.gz')
final_regulons_aucell_means_rss_plot_fname = os.path.join(data_folder, 'aucell_final_regulons_means_rss.pdf')
final_regulons_aucell_means_bin_fname = os.path.join(data_folder, 'aucell_final_regulons_means_binarized.csv.gz')
...
adata2 = adata2[adata2.obs['phase'] == 'G1']
adata2 = adata2[adata2.obs['celltype'] == 'leukemic']
adata2.obs['celltype'] = adata2.obs['batch']
...
metadata = pd.concat([adata.obs, adata2.obs], join='inner')
del adata, adata2, X1, X2
...
if __name__ == '__main__':
	...
	adata2 = ad.read_h5ad('/research/groups/allseq/data/scRNAseq/results/juha_wrk/ALL_scanpy/new_2019_ver5/subset/ALL_subset.h5ad')
	adata2 = adata2[adata2.obs['celltype'] == 'leukemic']
	adata2.obs['celltype'] = 'leukemic ' + adata2.obs['batch'].astype(str)
	adata2.obs['celltype'][adata2.obs['phase'] != 'G1'] = 'leukemic cycling'
	X1 = pd.DataFrame(adata.raw.X.toarray(), index=adata.obs.index, columns=adata.raw.var.index)
	X2 = pd.DataFrame(adata2.raw.X.toarray(), index=adata2.obs.index, columns=adata2.raw.var.index)
	data = pd.concat([X1, X2], join='inner')
	metadata = pd.concat([adata.obs, adata2.obs], join='inner')
	...
	for i in range(0, 4):
		for j in range(0, 4):
			plot_rss(rss, cts[i*4+j], ax=axs[i, j])
			if i != 4:
				axs[i, j].set_xlabel('')
			if j != 0:
				axs[i, j].set_ylabel('')
	plt.savefig(final_regulons_aucell_means_rss_plot_fname, bbox_inches='tight')
